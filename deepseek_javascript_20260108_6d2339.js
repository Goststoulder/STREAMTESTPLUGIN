(function () {
    ...
    function uaflix(component, _object) {
        ...
        function buildQualityMap(streams) {
            ...
            return { file: file, qualitys: qualitys, label: keys.length ? keys[0] : '' };
        }
    ...
    function uaserials(component, _object) {
        ...
        this.search = function (_object, data) {
            ...
            series = null;
            episodes_cache = {};
            if (isMovie(selected.type)) loadMovie(selected.id);
            else loadSeries(selected.id);
        };
        ...
        function search(title) {
            var url = api_base + '/search';
            var movie = object.movie || {};
            url = addParam(url, 'source', 'uaserials');
            url = addParam(url, 'title', title);
            url = addParam(url, 'original_title', movie.original_title || movie.original_name);
            url = addParam(url, 'imdb_id', movie.imdb_id);
            url = addParam(url, 'kinopoisk_id', movie.kinopoisk_id);
        ...
        function loadMovie(id) {
            var url = api_base + '/uaserials/movie';
            url = addParam(url, 'id', id);
        ...
        function loadSeries(id) {
            var url = api_base + '/uaserials/series';
            url = addParam(url, 'id', id);
        ...
        function buildEpisodes() {
            ...
            var season = series.seasons[choice.season] || series.seasons[0];
            var cache_key = voice.id + ':' + season;
            choice.voice_name = voice.name || voice.id;
            ...
            var url = api_base + '/uaserials/episodes';
        ...
        function getStream(item, success, fail) {
            var url = api_base + '/uaserials/stream';
            url = addParam(url, 'url', item.file);
    ...
    function uakino(component, _object) {
        ...
        function isMovie(category) {
            if (!category) return false;
            return /фільм|фильм|movie/i.test(category);
        }
        ...
        function search(title) {
            var url = api_base + '/search';
            var movie = object.movie || {};
            url = addParam(url, 'source', 'uakino');
            url = addParam(url, 'title', title);
            url = addParam(url, 'original_title', movie.original_title || movie.original_name);
            url = addParam(url, 'imdb_id', movie.imdb_id);
            url = addParam(url, 'kinopoisk_id', movie.kinopoisk_id);
        ...
        function loadSeries(id) {
            var url = api_base + '/uakino/series';
            url = addParam(url, 'id', id);
    ...
    function eneyida(component, _object) {
        ...
        function search(title) {
            var url = api_base + '/search';
            var movie = object.movie || {};
            url = addParam(url, 'source', 'eneyida');
            url = addParam(url, 'title', title);
            url = addParam(url, 'original_title', movie.original_title || movie.original_name);
            url = addParam(url, 'imdb_id', movie.imdb_id);
            url = addParam(url, 'kinopoisk_id', movie.kinopoisk_id);
        ...
        function loadMovie(id) {
            var url = api_base + '/eneyida/movie';
            url = addParam(url, 'id', id);
        ...
        function loadSeries(id) {
            var url = api_base + '/eneyida/series';
            url = addParam(url, 'id', id);
        ...
        function buildEpisodes() {
            ...
            var url = api_base + '/eneyida/episodes';
            url = addParam(url, 'id', selected.id);
            url = addParam(url, 'voice', voice.id);
            url = addParam(url, 'season', season);
        ...
        function getStream(item, success, fail) {
            var url = api_base + '/eneyida/stream';
            url = addParam(url, 'url', item.file);
    ...
    // Додаткові джерела можна додати аналогічно
    ...
    function component(object) {
        ...
        var sources = {
            ...
            uakino: uakino,
            eneyida: eneyida
            // Додайте тут інші джерела за аналогією
        };
        ...
        var balanser_titles = {
            ...
            uakino: 'UAKino',
            eneyida: 'Eneyida'
            // Додайте тут назви для нових джерел
        };
        ...
        var balanser_timer;
        var images = [];
        var filter_sources = ['uatut', 'uaflix', 'uaserials', 'uakino', 'eneyida']; // Оновлений список джерел
        ...
        this.similars = function (json) {
            ...
            json.forEach(function (elem) {
                ...
                if (elem.rating && elem.rating !== 'null' && elem.filmId) info.push(Lampa.Template.get('bandera_online_rate', { rate: elem.rating }, true));
                if (year) info.push(year);
                if (elem.countries && elem.countries.length) {
                    info.push((elem.filmId ? elem.countries.map(function (c) { return c.country; }) : elem.countries).join(', '));
                }
            ...
            this.getEpisodes(items[0].season, function (episodes) {
                ...
                items.forEach(function (element, index) {
                    ...
                    if (episode) {
                        ...
                        if (element.info.length < 30 && episode.vote_average) info.push(Lampa.Template.get('bandera_online_rate', { rate: parseFloat(episode.vote_average + '').toFixed(1) }, true));
                ...
                if (serial && episodes.length > items.length && !params.similars) {
                    ...
                    left.forEach(function (episode) {
                        var info = [];
                        if (episode.vote_average) info.push(Lampa.Template.get('bandera_online_rate', { rate: parseFloat(episode.vote_average + '').toFixed(1) }, true));
    ...
    function startPlugin() {
        ...
        var manifest = {
            ...
            name: '[Free] Bandera Online Extended',
        ...
        Lampa.Listener.follow('full', function (e) {
            if (e.type == 'complite') {
                ...
                btn.on('hover:enter', function () {
                    ...
                    Lampa.Activity.push({
                        url: '',
                        title: "Більше українських джерел",
                        component: 'bandera_online',
